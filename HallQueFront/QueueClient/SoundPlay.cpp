...
BOOL CSoundPlay::DataPlay(SLZData Data,BOOL bWait)
{
	...
	m_iPlayTimes = m_basicData.GetReCallNum();//呼叫次数
	if (bWait)
	{
...
// 	}
	int nWndAdd = 0;
	if(GetWndIdByData(Data,&nWndAdd))
		vst.iWndScreenId = nWndAdd;
...
{
	CString strPlaySound = m_basicData.GetCallMsg();
...
{
	CString strPlaySound = m_basicData.GetShowMsg();
...
BOOL CSoundPlay::GetWndIdByData(const SLZData& data,int *pAddress)
{
	BOOL flag = FALSE;
	list<CCallerWnd*>::const_iterator itera = m_list_callerWnd.begin();
	for(itera;itera != m_list_callerWnd.end();++itera)
	{
		...
		for(int i=0;i<3;i++)
		{
			if(pWnd->m_strCanDoBuss[i] == data.GetBussName())
			{
				flag = TRUE;
				*pAddress = pWnd->GetWndID();
				break;
			}
		}
